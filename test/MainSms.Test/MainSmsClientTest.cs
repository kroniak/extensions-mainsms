...
namespace MainSms.Test
{
    public class MainSmsClientTest
    {
        ...
        public async void SendBatchAsync()
        {
            ...
            {
                {"87000000001", "test message 1"},
                {"87000000002", "test message 2"}
            };
        ...
        public async void SendBatchErrorAsync()
        {
            ...
            var messages = new Dictionary<string, string>
            {
                {"89214045559", "test message 1"},
                {"89213020041", "test message 2"}
            };
        ...
        public async void ToReadme()
        {
            ...
            if (delayedSendResult.Status != "success" || !delayedSendResult.MessageIds.Any()) return;

            var cancelResult = await client.CancelAsync(delayedSendResult.MessageIds);
            if (cancelResult.Status != "success") return;

            if (!cancelResult.Statuses.Any()) return;
